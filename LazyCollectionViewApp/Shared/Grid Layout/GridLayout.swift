/// ⚠️ Do not use this layout, it is intended for demonstration purpose. Use `LazyVGrid` instead.

import SwiftUI
import LazyCollectionView

class GridLayout: LazyCollectionLayout, ObservableObject {
    var spacing: CGFloat = 16
    var padding: CGFloat = 16
    var minimumItemSize: CGSize = CGSize(width: 88, height: 88)
    ...
    func generateAttributes<Data>(for data: Data) where Data : Equatable, Data : RandomAccessCollection, Data.Element : Identifiable, Data.Index == Int {
        ...
        let elementsPerRow = Int(floor(contentWidth / (minimumItemSize.width + spacing)))
        let elementWidth = (contentWidth - Double(elementsPerRow-1) * spacing) / Double(elementsPerRow)
        let elementHeight = elementWidth * minimumItemSize.height / minimumItemSize.width

        let gridData = stride(from: 0, to: data.count, by: elementsPerRow).map {
            Array(data[$0..<min($0 + elementsPerRow, data.count)])
        }
        gridData.enumerated().forEach { (rowIndex, rowElements) in
            var elementOriginX = padding

            rowElements.enumerated().forEach { (columnIndex, element) in
                let frame = CGRect(x: elementOriginX,
                                   y: currentHeight,
                                   width: elementWidth,
                                   height: elementHeight).integral

                let index = rowIndex * elementsPerRow + columnIndex
                layoutAttributes.append(LazyCollectionLayoutAttributes(index: index + indexOffset, frame: frame))

                elementOriginX += elementWidth + spacing
            }
